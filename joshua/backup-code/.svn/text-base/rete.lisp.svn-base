...
(defmethod rete-topologizer ((type (eql :ignore)) data variables environment-stack)
  (progn type data variables environment-stack)	;meaning ignore
  (values))

(defmethod rete-topologizer ((type (eql :and)) data variables environment-stack)
  (progn type variables)			;meaning ignore
  (let ((analysis (make-and-group-pattern-analysis)))
    (loop with all-my-variables and entry and bottom-node and insertion and last-node
	  ...
	  when insertion
	    ...
	    and do (setq all-my-variables (union all-my-variables (pattern-analysis-variables-referenced insertion)))
...
(defmethod rete-topologizer ((type (eql :match)) data variables environment-stack)
  (progn type variables environment-stack)
...
(defmethod rete-topologizer ((type (eql :procedure)) data variables-in-block environment-stack)
  ...
  (let* ((code (car data))
	 ...
	 (analysis (make-procedure-pattern-analysis :variables-referenced all-my-variables
						    :name (second data)
						    :expression code
			    ...
			    when (member (car link) '(left right))
			      do (let ((brother (third link)))
				   (loop for variable in (pattern-analysis-variables-referenced brother)
					 when (member variable my-variables)
...
(defun write-rete-procedures (rule-name trigger-analysis &optional previous-map)
  (let ((type-of-trigger (type-of trigger-analysis)))
    (unless (eq type-of-trigger 'merge-pattern-analysis)
      (rete-code-writer (type-of trigger-analysis) rule-name trigger-analysis previous-map)))
  (loop with map = (pattern-analysis-map trigger-analysis)
	...
	do (cond ((and (eq (car link) 'right)
		       (not (pattern-analysis-map (second link)))
		       (pattern-analysis-map (third link)))
		  (let* ((left (third link))
			...
			(write-environment-mergers rule-name left-map map
						   (pattern-analysis-pure-semi-unification? trigger-analysis))
		      ...
		      (write-rete-procedures rule-name merge-structure))))
...
(defmethod rete-code-writer ((type (eql 'match-pattern-analysis)) rule-name analysis previous-map)
  (progn type previous-map)
...
(defmethod rete-code-writer ((type (eql 'and-group-pattern-analysis)) rule-name analysis previous-map)
  (progn type previous-map)
  (loop for trigger-analysis in (and-group-pattern-analysis-sub-patterns analysis)
	unless (pattern-analysis-map trigger-analysis)
	  do (write-rete-procedures rule-name trigger-analysis)))

(defmethod rete-code-writer ((type (eql 'or-group-pattern-analysis)) rule-name analysis previous-map)
  (progn type previous-map)
  (loop for sub-trigger in (or-group-pattern-analysis-sub-patterns analysis)
	unless (pattern-analysis-map sub-trigger)
	  do (write-rete-procedures rule-name sub-trigger)))

(defmethod rete-code-writer ((type (eql 'or-target-pattern-analysis)) rule-name analysis previous-map)
  (progn type rule-name previous-map)
  (when (null (pattern-analysis-map analysis))
    (setf (pattern-analysis-map analysis)
	  (build-map (intersection (pattern-analysis-variables-referenced analysis)
				   (pattern-analysis-variables-used analysis))
		     (pattern-analysis-name analysis)
		     ))))

;;; Write the trigger code for a procedure node
(defmethod rete-code-writer ((type (eql 'procedure-pattern-analysis)) rule-name analysis previous-map)
  (progn type)
  (let ((procedure-function-name (gentemp (format nil "~s-RULE-PROCEDURAL-TRIGGER-" rule-name))))
    (let ((post-procedure-map (write-forward-procedure-code procedure-function-name previous-map analysis)))
...
(defmethod rete-code-writer ((type (eql 'object-match-pattern-analysis)) rule-name analysis previous-map)
  (progn type previous-map)
  (multiple-value-bind (stuff semi map)
    ...
    (setf (object-match-pattern-analysis-semi-unification-code analysis) semi
	  (object-match-pattern-analysis-map analysis) map)))
...
(defmethod rete-net-constructor ((type (eql 'or-target-pattern-analysis)) rule-name trigger)
  (declare (special *node-making-code*))
  (progn rule-name)				;meaning ignore
