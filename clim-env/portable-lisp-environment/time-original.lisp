...
;;--- Note that this is stolen straight from KMP's time parser

(defconstant +whitespace+ '(#\Space #\Tab))

(proclaim '(inline whitespace-p))
...
(defmacro parse-string-tokens (kind (string items) &body forms)
  (clim-utils:with-gensyms (char token pos len type)
    (lispworks:rebinding (kind string)
      `(let* ((,pos 0)
              ,token
              (,len (loop for i downfrom (length ,string)
                          while (> i 0)
                          for j downfrom (1- i)
                          when (not (whitespace-p (char ,string j)))
		            do (return i)
                          finally (return 0)))
              (,char (when (> ,len 0) (aref ,string 0))))
         (macrolet ((next-char ()
                      `(progn
                         (incf ,',pos)
...
(defun parse-duration (string)
  (let (days weeks months years hours minutes seconds state)
