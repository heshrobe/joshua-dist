...
(define-command (com-interrupt-lisp-listener :command-table tools-menu
					     :name t :menu "Break LL") ()
  (let ((frames (navigator-frames *application-frame*)))
    (loop for frame in frames
	for process = (clim-internals::frame-top-level-process frame)
	when (and (typep frame 'lisp-listener) process)
	do (mp:process-interrupt process 'break)	     
	     )))
...
(defun shutdown-clim-environment (&key navigator (port (find-port)) framem)
  (let ((navigator (or navigator
    ...
    (when (and navigator
		...
		:foreground +white+
		:background (find-named-color "red" (frame-palette navigator))))
