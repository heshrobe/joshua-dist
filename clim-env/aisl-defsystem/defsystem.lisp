...
#+(or CCL-2 Allegro Minima Harlequin-Common-Lisp)	;Have to assume this won't blow up anybody's lisp
...
#+MCL
(load (make-pathname :directory (pathname-directory
				 (pathname-relative-to-loading-file NIL))
		     :name "mcl-patches-for-defsystem"
		     :type "lisp"))
...
(defpackage clim-defsystem
  (:use common-lisp)
  (:nicknames clim-defsys clim-ds aisl-defsys)
  (:export
    *current-system*
    *defsystem-version*
    ...
    *sysdcl-pathname-defaults*
    ...
    defsystem
    defsys
    #+LispWorks import-into-lispworks
...
(defun current-source-file ()
  ...
  #-(or Lucid Allegro xerox Genera Minima LispWorks) nil)
...
(defun compile-lisp-file (pathname binary-pathname optimizations)
  ...
  (ccl:set-mini-buffer ccl:*top-listener* "~A: Compiling ~A." 
                       *current-system*
                       (namestring pathname))
  (ensure-directories-exist binary-pathname :verbose T)
...
(defun compile-c-file (pathname binary-pathname compiler-flags)
  ...
  (let* ((command (format nil "cc~{ ~A~} -c -o ~a ~a"
	  ...
	  binary-pathname
	  pathname))
         (status (sys::call-system command)))
    (if (= status 0)
        t
        (error "Error compiling file ~a; status = ~s" pathname status)))
  #-(or ibcl (and Allegro unix) (and Lucid unix) (and LispWorks Unix))
...
(defun load-c-file (binary-pathname &optional libraries)
  ...
  #+(and Harlequin-Common-Lisp Unix (not Harlequin-PC-Lisp))
  (ffi::read-foreign-modules binary-pathname)
...
(defun define-system (system module-list)
  ...
  (intern-system system)
  #+LispWorks (import-into-lispworks (system-name system))
  system)


#+LispWorks (editor:setup-indent "defsystem" 2)
#+IGNORE
...
(defmacro in-module-env ((module) &body body)
  (let ((mod (gensym))
    ...
    `(let* ((,mod ,module)
		...
		:name "foo"
		:type (if (module-source-only ,mod)
                        "lisp"
                        #-(or LispWorks ppc-target) "fasl"
                        #+ppc-target "pfsl" 
                        #+LispWorks sys::*binary-file-type*)
...
verify that each already loaded subsystem is up-to-date, reloading it if need be."
  (let ((*loaded-systems* nil)
	(*tracep* trace))
    (load-subsystems (list (find-system name)) reload recurse source-if-newer query)
    name))

(defun load-subsystems (systems reload recurse source-if-newer query)
  (labels ((subsystems (s)
    ...
    (dolist (sys (expand-subsystems systems #'loadable-subsystems))
      (load-system-internal sys reload source-if-newer query))))

(defun load-system-internal (system reload source-if-newer query)
  (unless (member system *loaded-systems* :test #'eq)
	...
	(load-module module reload source-if-newer query))
...
(defun load-module (module reloadp source-if-newer query)
  (cond ((not (module-applicable-p module))
	...
	((or reloadp (not (module-up-to-date-p module source-if-newer)))
	 ...
	 (dolist (m (module-load-after-list module))
	   (load-module m reloadp source-if-newer query))
	 ;;
	 (let ((pathname (module-loadable-path module source-if-newer)))
	   (in-module-env (module)
	     (cond (*tracep* 
                    (fresh-line)
                    (format t ";;; -- Would load ~A" pathname))
		   (t 
                    (when (or (not query)
                              (y-or-n-p "Load ~A? " pathname))
                      (funcall (language-load-fn (find-language (module-language module)))
  ...
  (when (module-eval-after module)
    (in-module-env (module)
      (cond (*tracep*
	    ...
	    (t (eval (module-eval-after module)))))))
...
(defun compile-system (name &key (reload nil)
  ...
  (let ((*compiled-systems* nil)
	...
	(compile-subsystems (list (find-system name))
			    reload recompile propagate include-components query))
    name))

(defun compile-subsystems (systems reload recompile propagate include-components query)
  (labels ((subsystems (s)
	     (mapcar #'find-system
    ...
    (dolist (sys (expand-subsystems systems #'compilable-subsystems))
      (compile-system-internal sys reload recompile include-components query))))

(defun compile-system-internal (system reload recompile include-components query)
  (let ((name (system-name system))
	...
	(let ((*compiled-modules* nil)
          ...
          #+CCL-2 (ccl:set-mini-buffer ccl:*top-listener* "Compiling system ~A."
                                       *current-system*)
	  (dolist (module (system-module-list system))
	    (compile-module module reload recompile query #'pre-compile-fn)))
...
(defvar *load-all-before-compile* #-ALLEGRO NIL #+ALLEGRO T
  "If true, all previous modules will be loaded before a module is compiled.")

#+IGNORE
(defun compile-module (module reload recompile query pre-compile-fn)
  (flet ((file-write-date-or-nil (file)
	...
	(when (or recompile
	  ...
	  (if *load-all-before-compile*
		...
		(compile-module dep-module reload recompile query #'values)
		(load-module dep-module reload nil query))
	  ...
	  (in-module-env (module)
	    (cond (*tracep*
                   (fresh-line)
                   (format t ";;; -- Would compile ~A" s-pathname))
		  (t
		   (when (or (not query)
			     (y-or-n-p "Compile ~A? " s-pathname))
                     (funcall (language-compile-fn (find-language (module-language module)))
...
;;;--- At some point we need to work on translating the compile-order rules,
;;;--- but the present maximum-dependency rule will do to get started.
;;;--- jga 12 Sept 94
#+LispWorks
(defun import-into-lispworks (system-name)
  (let* ((top-system (or (lookup-system system-name)
	                 (error "No such CLIM system ~S" system-name)))
         (systems (expand-subsystems (list top-system)))
         (keyword-package (find-package "KEYWORD")))
    (labels ((process-features (features)
               (if (consp features)
                   (mapcar #'process-features features)
                   (cond ((member features '(and or not))
                          features)
                         ((and (symbolp features)
                               (not (eql (symbol-package features) keyword-package)))
                          (intern (symbol-name features) keyword-package))
                         (T
                          features)))))
      (loop for system in systems do
              (unless (scm:find-system (system-name system) NIL)
                (let* ((name (system-name system))
                       (pretty-name (string-upcase name))
                       (members (nconc
                                 ;;; NO.  A needed-system is NOT part of the system.
		                 ;;; (loop for system in (system-needed-systems system)
			         ;;;       collect `(,(string-downcase system) :type :system))
		                 (loop for module in (system-module-list system)
			               as language = (module-language module)
			               as name = (module-name module)
                                       as features = (module-features module)
			               collect
			               (ecase language
			                 (:lisp (if (and features
						         (not (eq features t)))
                                                    `(,name :features 
                                                            ,(process-features features))
                                                  name)))))))
	          (defsystem::ensure-system
	           pretty-name
                   name 
	           (system-default-pathname system)
                   nil ; default host
	           (system-default-binary-pathname system)
	           'defsystem:scm-system
                   (lw:current-pathname)
	           :default-type 'defsystem:scm-lisp-file
                   :members members
	           :package (find-package (system-default-package system))
...
(defun get-compiler-speed-and-safety ()
  ...
  #+LispWorks
  (values #-Harlequin-PC-Lisp 
          (compiler::optimization-level-speed compiler::*optimization-level*)
          #+Harlequin-PC-Lisp
          NIL
	  (compiler::optimization-level-safety compiler::*optimization-level*))
  #-(or Allegro Genera Lucid CCL-2 LispWorks) (values nil nil))

#+LispWorks (editor:setup-indent "with-compiler-options" 1)
...
;;;; MODIFICATIONS TO THE CLIM DEFSYSTEM ;;;;;;;;;;;;;;;;;;;;;;;;;;

(in-package :clim-defsystem)

;;;    Addition to improve functionality, clean up sources ;;;;;;;;

(defun map-system-modules (system function 
                                  &optional (desired-module-list (applicable-system-modules system)))
  (unless (system-p system)
    (setf system (find-system system)))
  (mapcar function desired-module-list))

(defun applicable-system-modules (system 
                                  &optional (test #'(lambda (module) (eq (module-language module) :lisp))))
  (remove-if-not
   ;; Only those modules which actually
   ;; apply and are Lisp files.
   #'(lambda (module)
       (and (funcall test module)
            (module-applicable-p module)))
   (system-module-list system)))

;;;    Modifications to support :source-only option for modules ;;;
...
pathname fields are evaluated."
  (flet ((expand-module-descr (name &key compile-satisfies-load (load-after t)
				    load-before-compile optimizations (language :lisp)
				    (features t) eval-after pathname source-only binary-pathname
				    package binary-only libraries)
	                      `(make-module :name ',(pretty-pathname-component (string name))
			                    :pathname ,(pretty-pathname-component pathname)
...
(defun module-loadable-path (module &optional source-if-newer)
  (let* ((source-only (module-source-only module))
         (bname (unless source-only (probe-file (module-bin-path module)))))
    (if (and (not source-only) bname (not source-if-newer))
	bname
      (let ((sname (probe-file (module-src-path module))))
        (cond ((and (null bname) (null sname))
               (error "Can't find source or binary for ~s" module))
              (source-only (if sname 
                               sname
                             (error "Can't find source for source-only module ~S" module)))
              ((null bname) (if (eq (module-language module) :lisp)
                                sname
                              (error "Can't find loadable version of ~s" module)))
              ((null sname) bname)
...
(defmacro defsys (name &rest module-list)
  "wrapper for defsystem to provide default-pathname/default-binary-pathname based on the name"
  (let* ((keywords (if (keywordp (caar module-list)) (pop module-list)))
	 (sysname (format nil "~A" name))
	 (default-pathname
	     (if (member :default-pathname keywords)
		 (second keywords)
		 (format nil "~A:" sysname)))
	 (default-binary-pathname (if default-pathname
				      (format nil "~A~A-binaries;"
					  default-pathname
					  #+MCL "MCL"
					  #+Allegro "ALLEGRO"
					  #+LispWorks "LISPWORKS"
					  ))))
    (unless (first module-list) (pop module-list))
    (unless (member :default-binary-pathname keywords)
      (push default-binary-pathname keywords)
      (push :default-binary-pathname keywords))
    (unless (member :default-pathname keywords)
      (push default-pathname keywords)
      (push :default-pathname keywords))
    `(defsystem ,name ,keywords ,@module-list)))

(defun compile-module (module reload recompile query pre-compile-fn)
  (flet ((file-write-date-or-nil (file)
	                         (if (probe-file file) (file-write-date file) nil)))
    (unless (or (not (module-applicable-p module))
                (module-source-only module)
		(module-binary-only module)
		(member module *compiled-modules* :test #'eq))
      (let* ((s-pathname (module-src-path module))
	     (b-pathname (module-bin-path module))
...
;;;    Addition of write-sytem-load-file capability ;;;;;;;;;;;;;;;

(defun write-system-load-file (system &key (logical T))
  (unless (system-p system)
    (setf system (find-system system)))
  (let* ((system-pathname (or (system-default-pathname system)
                              (system-defining-file system)))
         (system-host (pathname-host system-pathname))
         (up-pathname (make-pathname :host system-host
                                     :directory '(:relative :back)))
	 (default-pathname
          ;; The DEFSYSTEM:: version of MAKE-PATHNAME is broken!
          (lisp:make-pathname :defaults (merge-pathnames up-pathname system-pathname)
                              :name nil :type nil :version nil)))
    (with-open-file (stream (lisp:make-pathname :defaults (merge-pathnames 
                                                           (translate-logical-pathname default-pathname)
                                                           (make-pathname :directory '(:absolute)))
						:name (format NIL "load-file-for-~S" (system-name system)) ;; --- Unclear this will work on Unix.
						:type "")
			    :direction :output)
      (format stream ";;; LOAD FILE FOR ~A~%" (system-name system))
      (write-system-load-file-internal stream
                                       system
                                       (if (and (not logical) (typep default-pathname 'logical-pathname))
                                           (translate-logical-pathname default-pathname)
                                         default-pathname))
      (pathname stream))))

(defun write-system-load-file-internal (stream system default-pathname)
  (let ((*done-systems* nil)
        (*done-hosts* nil))
    (declare (special *done-systems* *done-hosts*))
    (write-load-file-list-for-system-internal stream system default-pathname)))

(defun write-load-file-list-for-system-internal (stream system default-pathname)
  (declare (special *done-systems* *done-hosts*))
  (when (member system *done-systems*)
    (return-from write-load-file-list-for-system-internal nil))
  (push system *done-systems*)
  (let ((pathname-host (pathname-host (or (system-default-pathname system)
                                          (system-defining-file system)))))
    (when (and pathname-host
               (typep default-pathname 'logical-pathname)
               (not (member pathname-host *done-hosts* :test #'equal)))
      (let ((comment-boundary (format NIL "~79,1,1,V@A" #\; ""))
            (comments (format NIL ";;; Edit this form if the logical host has a different location")))
        (format stream "~2%~A~%~A~2%(setf (logical-pathname-translations ~S)~%  '(~{~S~^~%   ~}))~%~A"
                comment-boundary
                comments
                pathname-host
                (logical-pathname-translations pathname-host)
                comment-boundary))
      (push pathname-host *done-hosts*)))
  (mapcan #'(lambda (system) (write-load-file-list-for-system-internal
                              stream system default-pathname))
          (mapcar #'find-system (system-needed-systems system)))
  (and (not (and (null (system-module-list system))
                 (null (system-patch-file-pattern system))))
       (labels ((module-src-bin (module)
                  (if (module-source-only module)
                    (module-src-path module)
                    (module-bin-path module))))
         (let* ((file-list-name (format nil "~(~A system files~)" (system-name system)))
                (module-loadable (if (typep default-pathname 'logical-pathname)
                                   #'(lambda (module) (module-src-bin module))
                                   #'(lambda (module) (translate-logical-pathname (module-src-bin module))))))
           (format stream "~2%;;; ~A~%~?~@[~%~A~]"
                   file-list-name
                   (format nil "~~@{(load ~~S)~~^~~%~~}")
                   (map-system-modules system #'(lambda (module)
                                                  (enough-namestring (funcall module-loadable module)
                                                                     default-pathname)))
                   (let ((pfp (system-patch-file-pattern system))
                         (bin-type (let ((type (second lisp-file-types)))
                                     (if (consp type) (first type) type))))
                     (and pfp
                          (format nil "(load ~A.~A)" pfp bin-type))))))))

(defun logical-pathname-p (pathname) (typep pathname 'logical-pathname))

;;;;  SETTING SYSTEM VARS

; (setf *sysdcl-pathname-defaults* "Gromit HD:Programming:Lisp:new defsystem:")

#+MCL (in-package :CCL) ; mcl doesn't make provide/require available from Common Lisp
(provide "AISL-DEFSYSTEM")
