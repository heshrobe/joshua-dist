;;; -*- Mode: Lisp; Package: JOSHUA-INTERNALS; Syntax: Joshua-ansi -*-
...
(define-predicate-model slot-value-mixin ((my-slot :accessor predication-my-slot))
...
(defmethod notice-new-value ((self basic-slot) value predication)
  (progn value predication)			;meaning ignore
  nil)
...
(defmethod act-on-new-value progn  ((self basic-slot) predication old-truth-value)
  (progn predication old-truth-value)		;meaning ignore
  nil)
...
(defmethod notice-new-value ((self set-valued-slot-mixin) value predication)
...
(defmethod insert-new-predication :around ((self truth-maintained-unique-valued-slot-mixin) predication value)
  (progn predication value)					;meaning ignore
  (with-slots (all-predications) self
...
(defmethod notice-new-value :before ((self value-overriding-unique-valued-slot-mixin) value predication)
  (with-slots (current-value current-predication) self
...
(defmethod notice-new-value ((self unique-valued-slot-mixin) value predication)
    ...
    (unless *building-prototype*
      (labels ((link-up (prototype object)
		 (let ((slot-names (all-slot-names prototype)))
		   ...
		   ;; Code here to check if I already have values and then to map-over-forward-rule
...
(defmethod basic-object-initializer ((self basic-object)) nil)
...
(defmethod notice-slot-change ((self basic-object) slot-name new-value justifying-predication)
  (progn self slot-name new-value justifying-predication)
  (values))
...
(defmethod initialize-instance :after ((self basic-object) &rest stuff)
  (declare (ignore stuff))
  (rebuild-object self)
...
(defmethod rebuild-object ((self basic-object))
   ...
   ((name &key slots parts equalities initializations included-object-types
	...
	(loop for s-d in slots
      ...
      (let ((initialization-tells
	      (loop with init-form-present and init-form
	 ...
	 ,@(when (or initialization-tells initializations equalities)
	     `((defmethod basic-object-initializer :after ((self ,name))
		 (with-slots ,slot-names self
                   (progn ,@slot-names)
...
(define-predicate-method (ask slot-value-mixin) (truth-value continuation do-backward-rules do-questions)
  ...
  (with-statement-destructured (path other-stuff) self
    (progn other-stuff)				;meaning ignore
...
(define-predicate-method (ask-data slot-value-mixin) (truth-value continuation)
  (progn truth-value)				;meaning ignore
...
(define-predicate-method (add-backward-rule-trigger slot-value-mixin :after) (truth-value trigger-object context rule-name)
  (progn trigger-object truth-value)		;meaning ignore
...
(define-predicate-method (locate-forward-rule-trigger type-of-mixin) (truth-value continuation context rule-name)
  (progn context rule-name)			;meaning ignore
  (unless (eql truth-value *true*)
    (error "this rule's pattern ~s has a truth value which isn't *true*" self))
  (with-statement-destructured (object type-name) self
    (progn object)				;meaning ignore
    (let ((type (object-type-named type-name)))
      (unless type
...
(define-predicate-method (write-forward-rule-semi-matcher type-of-mixin) (object-to-match environment)
  (progn environment)				;meaning ignore
  (with-predication-maker-destructured (my-object type) ()
    (progn type)				;meaning ignore
    (cond ((logic-variable-maker-p my-object)
  ...
  (with-statement-destructured (parent-object name child-object) self
	...
	(maphash #'(lambda (key child)
		 ...
		 (basic-object-subparts 
		  ;; parent is either provide or a pathname
...
(define-predicate-method (act-on-truth-value-change equated-mixin) (old-truth-value)
  (progn old-truth-value)			;meaning ignore
